...
// This wakes Render and lightly queries Supabase so the database stays active too.
app.get('/health', async (req, res) => {
  try {
    ...
    if (error) {
      console.log('❌ Health check Supabase failed:', error.message);

      return res.status(500).json({
        ok: false,
        service: 'pricepeek-server',
        render: 'awake',
        supabase: 'error',
        error: error.message,
